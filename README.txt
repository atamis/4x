...
Goal:
	...
	from humans. Harvest energy from nodes to warp in buildings and create
	spanning energy network. The goal of the game is to get 6 warpgates. This
	will create a large energy burst that rids the surface of the planet from
	any dangers.
...
Controls:
	...
	- After a unit is selected, it can do 4 different things accessible via
	  ...
	  button:
			...
			Build - This will open the buid submenu. Click which structure you
			want to build and the unit will alert the Warp Gate to warp in the
			building on its hex.
			...
			and all neighboring tiles. The scan action will use one of the
			units stamina.

			Purify - When clicked, the unit will lower the miasma level of the
			tile it stands on and all surrounding tiles by 1. This action takes
			2 stamina.
...
Buildings:
	...
	Conduits are used as a cheap way to spread the reach of your energy network.
	A valid energy connection is shown by a light blue line connecting buildings.

	Harvester - Generates 5 energy per turn when built on top of an energy node
	and 1 energy per turn when not. Connect this to your Warp Gates via Conduits.

	Purifier - A defensive structure that clears the Miasma off of one nearby
	tile per turn. Must be connected to an energy network to function. 

	Warp Gate - Warps in buildings and units at the expense of energy. Each Warp
	Gate can warp in one building at a time. Two Warp gates can warp in two
